...
static int buf_append(Memtable *m, FILE *segment, const void *data, size_t n) {
...
void flush(Memtable *m) {
  ...
  while (cur != 0 || sp > 0) {
    ...
    if (buf_append(m, segment, &n->key, sizeof(n->key)) != 0) { perror("buf_append key"); break; }
    if (buf_append(m, segment, &len, sizeof(len)) != 0)       { perror("buf_append len"); break; }
    if (len > 0) {
      if (!v->value) { fprintf(stderr, "flush: NULL value with len>0\n"); break; }
      if (buf_append(m, segment, v->value, (size_t)len) != 0) { perror("buf_append val"); break; }
    }
  ...
  m->next_segment_id = (id + 1) % (uint64_t)INT64_MAX;
  if (store_segment_count(m->next_segment_id) != 0) {
...
void mt_init(Memtable *m,RBNode *nodes, Value *values, int size, bool owns_values) {
  m->next_segment_id = load_segment_count();
  RBTree *t = &m->t;
...
bool mt_put(Memtable *m, long key, const char *value, int length){
  ...
  return rb_tree_put(&m->t,key,value,length);
}
